...
\end{equation}
Here, $\tilde{w}$ denotes the fixed, unique optimal vector of risky asset weights (normalized so they sum to 1), and $\tilde{w}^0$ dictates the mix between cash and this single optimal risky portfolio. In the literature, this unique purely risky portfolio $\tilde{w}$ is called the \textbf{Market Portfolio} or the \textbf{Tangency Portfolio}, denoted $V_M$.
\begin{equation*}
    V_M := \tilde{w} \cdot S.
\end{equation*}
